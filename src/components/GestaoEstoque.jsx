...
function GerenciadorDeEstoque() {
    ...
    async function salvarMovimentoDeEstoque() {
        ...
        let estoqueAtual = produtoEscolhido.rating.count;
        ...
        if (tipo === 'saida') {
            ...
            if (novoEstoque < ESTOQUE_MINIMO) {
                alert(`ATENÇÃO: O estoque do produto "${produtoEscolhido.title}" ficará abaixo do mínimo!`);
            }
            if (novoEstoque < 0) {
        ...
        try {
            ...
            let atualizarEstoque = await fetch('https://jsonplaceholder.typicode.com/posts', {
                method: 'POST',
                headers: {
                    ...
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title">
                                Movimentar Estoque: {produtoEscolhido?.title}
                            </h5>
                            <button type="button" className="btn-close" onClick={fecharJanelaDeMovimento}></button>
                        </div>
                        <div className="modal-body">
                            <p><strong>Estoque Atual:</strong> {produtoEscolhido?.rating.count}</p>
